...
def test_parse_fuzzy_list_none():
    assert parse_fuzzy_list(None) is None
...
def test_real_values():
    result = parse_fuzzy_list("'WooCommerce, Shopify, Magento, BigCommerce'")
    assert result == ['WooCommerce', 'Shopify', 'Magento', 'BigCommerce']
